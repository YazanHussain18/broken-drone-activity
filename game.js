...
class DroneViewer{
  ...
  buildDrone(){
    ...
    ['FL','FR','BL','BR'].forEach(key => {
      ...
      this.mesh(new THREE.CylinderGeometry(.13, .13, .08, 20), darker, prop, [x, .72, z]);
      const blade1 = this.mesh(new THREE.BoxGeometry(1.56, .045, .16), darker, prop, [x, .73, z], [0, .12, 0]);
      const blade2 = this.mesh(new THREE.BoxGeometry(1.56, .045, .16), darker, prop, [x, .73, z], [0, .12 + Math.PI / 2, 0]);
